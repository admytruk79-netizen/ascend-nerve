(()=>{
  ...
  const confirmHaptic=()=>{try{const Haptics=window.Capacitor?.Plugins?.Haptics;if(Haptics?.impact){Haptics.impact({style:'LIGHT'});return}}catch{}try{navigator.vibrate?.(10)}catch{}};
  ...
  async function complete(){if(user?.id){localStorage.setItem(localKey(user.id),'true');try{await PathBackend.completeIntroduction(user.id)}catch(error){console.warn('ASCEND introduction saved on this device',error)}}overlay.classList.add('hidden');overlay.setAttribute('aria-hidden','true');document.getElementById('app')?.removeAttribute('inert');document.querySelector('.bottom-nav')?.removeAttribute('inert');window.ASCENDUX?.activateScreen?.('today',{record:false});confirmHaptic()}
