(()=>{
  ...
  async function verifyPlayPurchase({productId,purchaseToken,productType}){if(!session?.access_token)throw new Error('Sign in required');const response=await fetch(`${BASE}/functions/v1/verify-play-purchase`,{method:'POST',headers:headers(),body:JSON.stringify({productId,purchaseToken,productType})});let body={};try{body=await response.json()}catch{}if(!response.ok)throw new Error(body.error||`Purchase verification failed (${response.status})`);return body}
  window.PathBackend={signInWithGoogle,listenForOAuthCallback,completeOAuth,signIn,signUp,resendSignup,requestPasswordReset,updatePassword,deleteAccount,verifyPlayPurchase,signOut,me,refresh,rest,rpc,redeemLifetimeKey,getMyEntitlement,loadCurriculum,ensureStudent,getProgress,completePractice,recordTrainingAssignment,saveJournal,getMarkerObservations,saveMarkerObservation,submitReadinessReview,mirrorSnapshot,getRecentJournalText,getMyProfile,getMyTeacher,getMyReviews,getMyStudents,getSharedJournalEntries,submitTeacherReview,isTeacher,addStudent,isSignedIn:()=>!!session?.access_token,isPasswordRecovery:()=>sessionStorage.getItem(RECOVERY)==='true'};
})();
