(()=>{
  ...
  async function verifyPlayPurchase({productId,purchaseToken,productType}){if(!session?.access_token)throw new Error('Sign in required');const body=await jsonFetch(`${BASE}/functions/v1/verify-play-purchase`,{method:'POST',body:JSON.stringify({productId,purchaseToken,productType})});if(!body?.verified)throw new Error(body?.reason||body?.error||'Google Play could not verify this purchase.');return body}
