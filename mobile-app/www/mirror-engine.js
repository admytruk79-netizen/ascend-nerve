(()=>{
 ...
 let scope='stage',stageId=null,loadedForStage='';
 ...
 </div>`;box.querySelectorAll('[data-mirror-scope]').forEach(b=>b.onclick=()=>load(b.dataset.mirrorScope,{force:true}));}
 ...
 async function load(nextScope=scope,{force=false}={}){scope=nextScope;const current=await resolveStage();if(!force&&scope==='stage'&&current&&loadedForStage===current)return;loading();try{const data=await invoke(scope);loadedForStage=scope==='stage'?(stageId||current||''):'';render(data)}catch(e){const box=document.getElementById('mirror-content');if(box)box.innerHTML=`<p>${esc(e.message)}</p><p class="mirror-boundary">Your journal remains intact. Mirror does not alter progression or readiness.</p>`}}
 function wire(){const card=document.querySelector('#me .rhythm-card:has(#mirror-content)');const heading=card?.querySelector('h2');if(heading)heading.textContent='Mirror · Resonance';const old=document.getElementById('refresh-mirror');if(old&&old.dataset.resonance!=='1'){old.dataset.resonance='1';old.textContent='Read Resonance';old.addEventListener('click',()=>load(scope,{force:true}))}}
 document.addEventListener('ascend:navigation',event=>{if(event.detail?.to==='me')load('stage')});
 document.addEventListener('ascend:journal-saved',()=>{loadedForStage=''});
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',wire,{once:true});else wire();
 window.ASCENDMirror={load,invalidate:()=>{loadedForStage=''}};
})();