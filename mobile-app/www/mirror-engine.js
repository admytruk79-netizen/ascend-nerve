(()=>{
  ...
  const CACHE_PREFIX='ascendMirrorCache:';
  ...
  const cacheKey=nextScope=>`${CACHE_PREFIX}${nextScope}`;
  const readCache=nextScope=>{
    try{
      const cached=JSON.parse(localStorage.getItem(cacheKey(nextScope))||'null');
      if(!cached?.data||Date.now()-Number(cached.saved_at||0)>CACHE_TTL)return null;
      return cached.data;
    }catch{return null}
  };
  const writeCache=(nextScope,data)=>{
    try{localStorage.setItem(cacheKey(nextScope),JSON.stringify({saved_at:Date.now(),data}))}catch{}
  };
  const invalidate=()=>{
    try{
      localStorage.removeItem(cacheKey('stage'));
      localStorage.removeItem(cacheKey('all'));
    }catch{}
  };
  ...
  async function requestResonance(nextScope,token){
  ...
  function render(data,{cached=false}={}){
    ...
    box.innerHTML=`
      <div class="mirror-shell">
        ...
        <div class="mirror-status"><span>RESONANCE ENGINE</span><small>${cached?'CACHED · ':''}${esc((data.confidence||'insufficient').toUpperCase())} SIGNAL · ${m.journal_entries||0} ENTRIES</small></div>
  ...
  function showCached(nextScope=scope){
    scope=nextScope;
    const cached=readCache(nextScope);
    if(cached){render(cached,{cached:true});return true}
    const box=document.getElementById('mirror-content');
    if(box)box.innerHTML='<p>Mirror is ready when you are. Read Resonance to reflect on patterns in your journal.</p>';
    return false;
  }

  function setButtonState(active){
    const btn=document.getElementById('refresh-mirror');if(!btn)return;
    btn.disabled=active;
    btn.textContent=active?'Reading…':'Read Resonance';
    btn.setAttribute('aria-busy',String(active));
  }

  function loading(){
    const box=document.getElementById('mirror-content');
    if(box)box.innerHTML='<div class="mirror-loading" role="status" aria-live="polite"><i></i><p>Reading resonance across your record…</p></div>';
  }

  function renderError(message){
    const box=document.getElementById('mirror-content');if(!box)return;
    const cached=readCache(scope);
    if(cached){render(cached,{cached:true});return}
    box.innerHTML=`<p>${esc(message)}</p><p class="mirror-boundary">Your journal remains intact. Mirror stays on this screen.</p><button class="secondary mirror-reconnect" type="button">Reconnect Mirror</button>`;
  }
  ...
  async function load(nextScope=scope){
    ...
    busy=true;
    setButtonState(true);
    const hadCache=showCached(scope);
    if(!hadCache)loading();
    try{
      const data=await invoke(scope);
      writeCache(scope,data);
      render(data);
    }catch(e){
      renderError(e?.message||'Mirror could not read resonance right now.');
    }finally{
      busy=false;
