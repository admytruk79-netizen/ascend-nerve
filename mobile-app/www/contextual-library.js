(()=>{
  ...
  function relatedItems(){
    ...
    const picked=wanted.map(slug=>c.content.find(x=>x.slug===slug)).filter(Boolean);
    ...
    return c.content.map(item=>({item,score:words.reduce((n,w)=>n+(((item.title||'')+' '+(item.summary||'')).toLowerCase().includes(w)?1:0),0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,3).map(x=>x.item);
  }
  ...
  function makeButton(item,context){
    ...
    b.innerHTML=`<span class="related-teaching-mark">✦</span><span class="related-teaching-copy"><small>${context==='practice'?'RELATED TEACHING':'FOR THIS STAGE'}</small><strong>${esc(item.title)}</strong></span><span class="related-teaching-arrow">›</span>`;
    b.addEventListener('click',()=>openItem(item));return b;
  }

  function render(){
    const items=relatedItems();if(!items.length)return;
  ...
  let lastKey='';
  function tick(){
    const key=currentStageTitle()+'|'+(window.curriculum?.content?.length||0);
    if(key!==lastKey&&window.curriculum?.content?.length){lastKey=key;render()}
    setTimeout(tick,500);
  }
