...
let user=null,curriculum=null,currentStage=null,currentPractice=null,progressRow=null,markerObservations=[],recentJournalText='',libraryQuery='',libraryType='all';
...
async function refreshAccess(){const label=document.getElementById('access-level'),form=document.getElementById('lifetime-key-form');if(!label||!user)return;try{const entitlement=await PathBackend.getMyEntitlement(user.id);const lifetime=entitlement?.access_level==='lifetime';label.textContent=lifetime?'Lifetime Access':entitlement?.access_level==='premium'?'Premium Access':'Free Access';label.classList.toggle('lifetime',lifetime);form?.classList.toggle('hidden',lifetime)}catch(e){console.error(e);label.textContent='Access unavailable'}}
function escapeHtml(s=''){return String(s).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}

nav.forEach(b=>b.addEventListener('click',()=>{nav.forEach(x=>x.classList.remove('active'));b.classList.add('active');screens.forEach(s=>{const active=s.id===b.dataset.screen;s.classList.toggle('active',active);if(!active)s.classList.remove('motion-enter');s.setAttribute('aria-hidden',String(!active))})}));
...
function renderLibrary(){if(!curriculum)return;
  ...
  listLabel?.classList.toggle('hidden',!hasPicks);
}
...
async function loadRemote(){try{user=await PathBackend.me();showAccount();setSync('LOADING…');curriculum=await PathBackend.loadCurriculum();window.__pathProgress=[];recentJournalText='';markerObservations=[];if(user){await PathBackend.ensureStudent(user);await refreshAccess();window.__pathProgress=await PathBackend.getProgress(user.id);recentJournalText=await PathBackend.getRecentJournalText(user.id).catch(()=>'')}progressRow=window.__pathProgress.find(p=>p.status==='active'||p.status==='review')||window.__pathProgress[window.__pathProgress.length-1]||null;currentStage=curriculum.stages.find(s=>s.id===progressRow?.stage_id)||curriculum.stages[0]||null;if(user&&currentStage)markerObservations=await PathBackend.getMarkerObservations(user.id,currentStage.id);renderStage();await refreshMirror();window.ASCENDBranches?.load?.();window.ASCENDTrainingLayers?.load?.();window.ASCENDReadinessEvidence?.load?.();setSync(user?'SYNCED':'READY',true)}catch(e){console.error(e);setSync('LOCAL');showAccount()}}

const overlay=document.getElementById('practice-overlay');document.querySelector('[data-action="practice"]').addEventListener('click',()=>{resetTimerUI();overlay.classList.remove('hidden')});document.querySelector('.overlay-close').addEventListener('click',()=>overlay.classList.add('hidden'));
